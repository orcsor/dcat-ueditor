...
use Orcsor\DcatUeditor\Events\Catched;
use Orcsor\DcatUeditor\Events\Uploaded;
...
class Storage
{
    ...
    {
        if (!$disk) {
            $disk = config('ueditor.disk') ?: config('admin.upload.disk', 'public');
        }
    ...
    protected function fileHasError(UploadedFile $file, array $config)
    {
        ...
        if (!$file->isValid()) {
            $error = $file->getError();
        } else if ($file->getSize() > $config['max_size']) {
            $error = 'upload.ERROR_SIZE_EXCEED';
        } else if (!empty($config['allow_files']) &&
